pipeline {
  ...
  environment {
    BOT_NAME = 'briefing_bot'
    RELEASE_DIR = "/opt/ergon/releases/${BOT_NAME}"
    GITHUB_REPO = "ergon-automation-labs/ergon-briefing"
  }
    ...
    stage('Run Migrations') {
      steps {
        sh '''
          ...
          RELEASE_BIN="${RELEASE_DIR}/current/briefing_bot/bin/briefing_bot"
          ...
          echo "Running: $RELEASE_BIN eval 'BotArmyBriefingBot.Release.create()'"
          $RELEASE_BIN eval 'BotArmyBriefingBot.Release.create()'

          echo "Running: $RELEASE_BIN eval 'BotArmyBriefingBot.Release.migrate()'"
          $RELEASE_BIN eval 'BotArmyBriefingBot.Release.migrate()'
  ...
  post {
    success {
      sh '''
        ...
        if [ -f ./release-artifact/bot_army_briefing_bot/releases/start_erl.data ]; then
          VERSION=$(awk '{print $2}' ./release-artifact/bot_army_briefing_bot/releases/start_erl.data)
        fi
